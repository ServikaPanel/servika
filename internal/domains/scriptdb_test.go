...
// answered by the fragment of its text that the script names, and every
// statement is recorded with its arguments.
//
...
// stepsSnapshot returns the recorded steps.
func (s *sqlScript) stepsSnapshot() []string {
	s.mu.Lock()
	defer s.mu.Unlock()
