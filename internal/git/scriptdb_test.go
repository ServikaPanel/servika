...
// A scripted database, in the shape internal/domains uses. A query is answered
...
type sqlScript struct {
	...
	// affected maps a fragment to the row count its statement reports, for the
...
func (s *sqlScript) exec(query string, args []driver.NamedValue) (driver.Result, error) {
	...
	rows := int64(1)
	for named, count := range s.affected {
		if strings.Contains(query, named) {
			rows = count
		}
	}
	return sqlScriptResult{rows: rows}, nil
}
