...
func parseCrontab(r io.Reader) ([]Task, error) {
	reader := crontabReader{out: make([]Task, 0)}
	sc := bufio.NewScanner(r)
	for sc.Scan() {
		reader.read(strings.TrimSpace(sc.Text()))
	}
	return reader.out, sc.Err()
}

// crontabReader is what the parse carries from one line to the next: the tasks
// so far, and the comment and metadata line that belong to the NEXT task.
type crontabReader struct {
	out         []Task
	lastComment string
...
// read takes one trimmed line of the crontab.
func (c *crontabReader) read(line string) {
	if line == "" {
		c.lastComment = ""
		c.meta = nil
		return
	}
	text, found := strings.CutPrefix(line, "#")
	if !found {
		c.add(line, false)
		return
	}
	c.readComment(strings.TrimSpace(text))
}

// readComment takes a line that began with '#', which is three different things
// in a panel-written crontab.
func (c *crontabReader) readComment(text string) {
	// The panel's own metadata comment for the NEXT task.
	if rest, ok := strings.CutPrefix(text, metaPrefix); ok {
		c.meta = parseMeta(strings.TrimSpace(rest))
		return
	}
	// A disabled task is a commented-out cron line: tell it apart from a human
	// comment by whether what follows the '#' looks like a cron schedule (five
	// schedule fields then a command).
	if looksCron(text) {
		c.add(text, true)
		return
	}
	// Skip the banner generated by this package; keep any other comment.
	if !strings.HasPrefix(text, "servika") {
		c.lastComment = text
	}
}
...
func (c *crontabReader) add(line string, disabled bool) {
	...
	task := Task{
		...
		Comment: c.lastComment,
		Enabled: !disabled,
		Type:    TypeCommand,
	}
	if c.meta != nil {
		applyMeta(&task, c.meta)
	}
	c.out = append(c.out, task)
	c.idx++
	c.lastComment = ""
	c.meta = nil
}

// applyMeta fills the rich fields a metadata comment carries.
func applyMeta(task *Task, meta map[string]string) {
	if v := meta["type"]; v != "" {
		task.Type = v
	}
...
func serializeCrontab(systemUser string, domainID int64, secret []byte, list []Task) []byte {
	...
	for _, task := range list {
		meta, runCommand := taskLine(domainID, secret, task)
		if len(meta) > 0 {
...
// taskLine returns one task's metadata fields and the command its cron line
// runs.
//
// The metadata carries only the fields that differ from the defaults, so a
// plain enabled command task writes no metadata line at all.
func taskLine(domainID int64, secret []byte, task Task) (meta []string, runCommand string) {
...
// The checks run in the order they were written in, because a task that breaks
// two rules must keep reporting the same one.
func validate(task Task) error {
	if err := schedulePresent(task); err != nil {
		return err
	}
	if err := commandLength(task.Command); err != nil {
		return err
	}
	if err := scheduleCharacters(task); err != nil {
		return err
	}
...
// schedulePresent reports the one refusal an incomplete schedule gets.
func schedulePresent(task Task) error {
...
// commandLength bounds the command that will be written onto the cron line.
func commandLength(command string) error {
	if command == "" {
...
// scheduleCharacters refuses anything in a schedule field that would end the
// crontab line or start a second command on it.
func scheduleCharacters(task Task) error {
...
func buildCommand(in taskInput) (string, error) {
	switch in.Type {
	case TypeURL:
		return urlCommand(in.URL)
	case TypePHP:
		return phpCommand(in)
	default: // TypeCommand or empty
		return in.Command, nil
	}
}

// urlCommand builds the curl line of a URL task.
func urlCommand(raw string) (string, error) {
...
// phpCommand builds the interpreter line of a PHP task.
func phpCommand(in taskInput) (string, error) {
