...
# Every ops tool carries its own copy of this loader on purpose. A sourced
# library (see servika-offsite-lib) is best effort, because a release where
# it has not landed yet must still work; reading the environment cannot be.
# A Go test fails the build if these parse loops ever drift apart.
load_servika_env() {
  [ -f "$ENV_FILE" ] || return 0
