...
// SVG plane — uniform scale
const W0 = 1000, H = 300, ML = 40, MR = 16, MT = 16, MB = 28
const COLOR = '#0ea5e9' // sky — visually distinct from the CPU chart
...
export default function MemoryHistoryChart() {
  ...
  const g = useMemo(() => {
    ...
    const line = smoothPath(pts, xAt, (p) => yAt(p.memory))
  ...
  const currentValue = hp ? hp.memory : latest ? latest.memory : null
...
// Monotone-cubic smoothing (stays above baseline) — same method as LoadHistoryChart
function smoothPath(data: Point[], xAt: (i: number) => number, yAt: (p: Point) => number): string {
