    ...
    <div className="px-4 py-4 sm:px-6 sm:py-5">
      ...
      {job && (
        <>
          ...
          <JobItemsCard job={job} detail={detail} />
        </>
      )}
    </div>
  )
...
// JobHeader names the operation, its id and who started it.
function JobHeader({ job }: { job: Job }) {
  const { t } = useTranslation('BackupManagementPage')
  const restore = job.operation === 'restore'
  return (
    <>
      <div className="flex items-center gap-3 mb-1">
        <span><Icon d={restore ? ICON.refresh : ICON.save} className="h-6 w-6" /></span>
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">
          {t(restore ? 'jobs.opRestore' : 'jobs.opBackup')} #{job.id}
        </h1>
      </div>
...
// JobProgress counts the items the job has settled so far.
function JobProgress({ job, running }: { job: Job; running: boolean }) {
  const { t } = useTranslation('BackupManagementPage')
...
// JobItemsCard lists the domains of a backup job, or the outcome of a restore.
function JobItemsCard({ job, detail }: { job: Job; detail: Detail | null }) {
  const { t } = useTranslation('BackupManagementPage')
  return (
