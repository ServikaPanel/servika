...
// ResultNotice reports what the last write applied, skipped and rejected.
function ResultNotice({ result }: { result: WriteResult | null }) {
...
// AddCard takes one or more hostnames and the scope they are banned in.
function AddCard({ domains, onDomains, description, onDescription, matchSubdomains, onMatchSubdomains, saving, onSubmit }: {
  domains: string
  onDomains: (value: string) => void
  description: string
  onDescription: (value: string) => void
  matchSubdomains: boolean
  onMatchSubdomains: (value: boolean) => void
  saving: boolean
  onSubmit: () => void
}) {
  const { t } = useTranslation('BannedDomainsPage')
  return (
    <div className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/40">
      ...
      <textarea
        ...
        rows={4}
        value={domains}
        onChange={event => onDomains(event.target.value)}
      ...
      <label className="mt-3 flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
        <input
          ...
          onChange={event => onMatchSubdomains(event.target.checked)}
...
// BannedTable lists the hostnames currently banned.
function BannedTable({ rows, loading, saving, onRemove }: {
  rows: BannedDomain[]
  loading: boolean
  saving: boolean
      ...
      <table className={responsiveTableClass}>
        ...
        <tbody className={responsiveTableBodyClass}>
          ...
          {!loading && rows.map(row => (
            <BannedRow key={row.domain} row={row} saving={saving} onRemove={onRemove} />
          ))}
        </tbody>
      </table>
    </div>
...
// BannedRow is one banned hostname of that table.
function BannedRow({ row, saving, onRemove }: {
  row: BannedDomain
  saving: boolean
  onRemove: (domain: string) => void
}) {
...
export default function BannedDomainsPage() {
  ...
  return (
    <div className="px-4 py-4 sm:px-6 sm:py-5">
      ...
      <AddCard
        domains={domains} onDomains={setDomains}
        description={description} onDescription={setDescription}
        matchSubdomains={matchSubdomains} onMatchSubdomains={setMatchSubdomains}
        saving={saving} onSubmit={addDomains}
      />
      ...
      <BannedTable rows={visible} loading={loading} saving={saving} onRemove={removeDomain} />
