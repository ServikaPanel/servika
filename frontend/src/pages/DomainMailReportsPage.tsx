...
function MailboxNotices({ status, id }: { status: Status | null; id?: string }) {
...
function ScanLine({ status }: { status: Status | null }) {
  const { t } = useTranslation('DomainMailReportsPage')
...
function TabBar({ tab, onTab, days, onDays }: {
  tab: Tab; onTab: (value: Tab) => void; days: number; onDays: (value: number) => void
}) {
  const { t } = useTranslation('DomainMailReportsPage')
  ...
  return (
    <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
      ...
      {tab !== 'mtasts' && (
        <div className="inline-flex gap-1 rounded-xl bg-slate-100 p-1 dark:bg-slate-800">
          {WINDOWS.map(window => (
            <button key={window} type="button" onClick={() => onDays(window)}
...
function DmarcCard({ sources }: { sources: Source[] }) {
  const { t } = useTranslation('DomainMailReportsPage')
  return (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {sources.map(source => <SourceRow key={source.source_ip} source={source} />)}
            </tbody>
          </table>
        </div>
      )}
...
function TlsCard({ tls }: { tls: TLSSummary | null }) {
  const { t } = useTranslation('DomainMailReportsPage')
  if (!tls) return null
  return (
...
function MtastsSteps({ mtasts }: { mtasts: MTASTS }) {
  const { t } = useTranslation('DomainMailReportsPage')
  // The sequence waits on the world, so the screen names the step it is on
  // instead of showing a spinner that never resolves.
  if (mtasts.mode !== 'pending_dns' && mtasts.mode !== 'pending_cert') return null
  return (
...
function MtastsActions({ mtasts, saving, onMode, onWithdraw, onRefresh }: {
  mtasts: MTASTS; saving: boolean
  onMode: (mode: 'testing' | 'enforce') => void; onWithdraw: () => void; onRefresh: () => void
}) {
  const { t } = useTranslation('DomainMailReportsPage')
  return (
    <div className="mt-5 flex flex-wrap gap-2">
      {mtasts.mode === 'off' && (
        <button type="button" disabled={saving} onClick={() => onMode('testing')}
          className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50">
          {t('mtasts.actions.enable')}
        </button>
      )}
      {mtasts.mode === 'testing' && (
        <button type="button" disabled={saving || mtasts.enforce_blocked} onClick={() => onMode('enforce')}
          className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50">
          {t('mtasts.actions.enforce')}
        </button>
      )}
      {mtasts.mode !== 'off' && mtasts.mode !== 'withdrawing' && (
        <button type="button" disabled={saving} onClick={onWithdraw}
...
function MtastsNotices({ mtasts }: { mtasts: MTASTS }) {
  const { t } = useTranslation('DomainMailReportsPage')
  const reasonKey = ENFORCE_REASONS[mtasts.enforce_reason || ''] || 'mtasts.reasons.unknown'
  return (
    <>
      {/* The lock is explained rather than left as a greyed-out button: enforce
          is the one control here that can stop mail being delivered, so the
          reason has to be readable. */}
      {mtasts.mode === 'testing' && mtasts.enforce_blocked && (
        <p className="mt-3 text-xs text-amber-700 dark:text-amber-300">{t(reasonKey)}</p>
      )}
...
function MtastsCard({ mtasts, saving, onMode, onWithdraw, onRefresh }: {
  mtasts: MTASTS | null; saving: boolean
  onMode: (mode: 'testing' | 'enforce') => void; onWithdraw: () => void; onRefresh: () => void
}) {
  const { t } = useTranslation('DomainMailReportsPage')
  if (!mtasts) return null
  return (
    <div className="mt-5 space-y-5">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        ...
        <MtastsActions mtasts={mtasts} saving={saving} onMode={onMode} onWithdraw={onWithdraw} onRefresh={onRefresh} />
        <MtastsNotices mtasts={mtasts} />
      </div>
    </div>
  )
}
...
export default function DomainMailReportsPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <MailboxNotices status={status} id={id} />

      {loading ? (
        <p className="mt-6 text-sm text-slate-500 dark:text-slate-400">{t('loading')}</p>
      ) : (
        <>
          <TabBar tab={tab} onTab={setTab} days={days} onDays={setDays} />

          <ScanLine status={status} />

          {tab === 'dmarc' && <DmarcCard sources={sources} />}

          {tab === 'tlsrpt' && <TlsCard tls={tls} />}

          {tab === 'mtasts' && (
            <MtastsCard mtasts={mtasts} saving={isSaving} onMode={setMode} onWithdraw={withdraw} onRefresh={reload} />
          )}
        </>
      )}
    </div>
