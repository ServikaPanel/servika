...
/**
 * The admin/reseller view of one domain's findings, reached from the security
 * monitor. It reuses the scoped admin list endpoint with a domain_id filter, so
 * a reseller opening a row still sees only what they own.
 */
export default function AdminDomainSecurityPage() {
  ...
  const load = useCallback(() => {
    if (!id) return
    api.get<SecurityFinding[]>(`/admin/site-security?domain_id=${id}`)
  ...
  const domainName = findings[0]?.domain_name

  return (
    <div className="px-4 py-4 sm:px-6 sm:py-5">
      <Breadcrumb items={[
        { label: t('breadcrumb.home'), href: '/' },
        { label: t('breadcrumb.current'), href: '/site-security' },
        { label: domainName || t('domainTable.domain') },
      ]} />
