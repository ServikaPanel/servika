...
func main() {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.With(middleware.AdminOnly).Get("/system/host-apps/metrics", hostAppH.Metrics)
			...
			r.With(middleware.AdminOnly).Get("/system/host-apps/{id}/backups", hostAppH.Backups)
			...
			r.Group(func(r chi.Router) {
				...
				r.With(middleware.CustomerScope).Get("/domains/{id}/apps/{aid}/metrics", appsH.Metrics)
				...
				r.With(middleware.CustomerScope).Get("/domains/{id}/apps/{aid}/backups", appsH.BackupList)
				r.With(middleware.CustomerScope).Post("/domains/{id}/apps/{aid}/backups", appsH.BackupCreate)
				r.With(middleware.CustomerScope).Post("/domains/{id}/apps/{aid}/backups/{bid}/restore", appsH.BackupRestore)
				r.With(middleware.CustomerScope).Delete("/domains/{id}/apps/{aid}/backups/{bid}", appsH.BackupDelete)
...
// `go build` run has no such directory, so nothing migrates from a checkout.
const migrationsDir = "/opt/servika/src/migrations"

// runMigrations applies each migration file exactly once, tracked in the
// schema_migrations table. A failure is fatal: startup stops rather than letting
// a later migration run on a schema whose history is unknown. The runner itself
// lives in internal/dbmigrate, which resumes a file that stopped part way
