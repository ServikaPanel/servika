...
| `SERVIKA_DB_DSN`           | none, required                                            | MariaDB DSN for the `panel` database. There is no built-in default, so the server cannot start on a shared fallback credential. `scripts/seed_admin.go` reads it too, so the DSN never has to be a `-dsn` argument. |
...
| `SERVIKA_SEED_PASSWORD`     | empty                                                                | `scripts/seed_admin.go`, `servika-install.sh`            | Administrator password fallback when `-password` is omitted; the installer always uses it, so the password never reaches argv. |
...
| `SERVIKA_HEALTH`            | derived from `SERVIKA_LISTEN`                                        | `servika-update`, `servika-restore`                      | Health check URL used after restoring core files. Derived so that moving the panel's backend port cannot leave it pointing at a dead one. |
...
| `SERVIKA_MYSQL_SOCKET`          | `/var/lib/mysql/mysql.sock`                        | Where MariaDB listens locally. It is the only way to reach a tenant database account: the panel creates every one as `'user'@'localhost'`, and MariaDB treats a TCP connection to 127.0.0.1 as a different host and refuses it. |
| `SERVIKA_AV_CACHE_FILE`         | `/var/lib/servika/av/rapidscan.json`               | What the last server sweep read and found clean, so the next one skips a file whose size, mtime and ctime all still match. Discarded whole whenever the binary, the rule package or a detection layer changes, which costs one full sweep. |
...
- **Go 1.26.6+** for the backend (`go.mod` declares `go 1.26.6`; the default `GOTOOLCHAIN=auto` fetches it automatically, and an older local toolchain is refused rather than used)
...
```bash
SERVIKA_DB_DSN='<DSN>' SERVIKA_SEED_PASSWORD='CHOOSE_A_PASSWORD' \
  go run scripts/seed_admin.go -username admin -email 'admin@example.com'
# The -dsn and -password flags still work, but they put both credentials in
